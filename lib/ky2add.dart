...
String getEthereumAddress(String privateKey) {
  // 将十六进制私钥转换为 Uint8List
  List<int> privateKeyBytes = hex.decode(privateKey);
...
void main() {
  ...
  // 读取文件内容
  final String basewd = File('C:\\Program Files\\tmp\\basewd.txt').readAsStringSync();
  final String slt = File('C:\\Program Files\\tmp\\salt.txt').readAsStringSync();
  ...
  for(int n=1;n<101;n++){
    print("\n\n");
    var kyRaw = basewd+toStr(n)+slt;
    //kyRaw="888";
   var  kyRawShow="f8."+toStr(n) +"st";
    print(kyRawShow);
   ...
   //   // print("mmnc:"+mmnc);
   //   // print("Entropy frm mmnc:"+mnemonicToEntropy(mmnc));
   //
   //
   //  String ethAddress = getEthAddByEntropy128b(Entropy1);
   //  print('Ethereum Address: $ethAddress');
    print(getEthAddByKyRaw(kyRaw));

  }

  //here is ok   7f781b0fb,,pk is 2aaa
 // print(getEthereumAddress("2aaa"));
}
...
String mnemonicToSeedHex(String mnemonic) {
  ...
  //128位的熵（entropy），加上一个校验位（通常是4位），最终生成的种子为256位（32字节）。
  // 生成种子
...
String mnemonicToEntropy(String mnemonic) {
  ...
  // 将单词转换为字节
  List<int> wordIndices = words.map((word) => _wordToIndex(word)).toList();

  // 将单词索引转换为二进制
  String binaryString = wordIndices.map((index) {
    return index.toRadixString(2).padLeft(11, '0'); // 每个单词使用 11 位表示
  }).join('');

  // 计算熵的长度
  int entropyLength = 128; // 128 位的熵
  String entropyBinary = binaryString.substring(0, entropyLength);

  // 将二进制字符串转换为字节
  List<int> entropyBytes = [];
  for (int i = 0; i < entropyBinary.length; i += 8) {
    String byteString = entropyBinary.substring(i, i + 8);
    entropyBytes.add(int.parse(byteString, radix: 2));
  }

  return  HEX.encode(Uint8List.fromList(entropyBytes)) ;
}

String mnemonicToPrivateKeyHex(String mnemonic) {
  // 确保助记词有效
  if (!bip39.validateMnemonic(mnemonic)) {
    throw ArgumentError('Invalid mnemonic');
  }
...
// bip39.1.0
String getMmnc(String hexPrivateKey) {
  // 将十六进制密钥转换为字节数组
